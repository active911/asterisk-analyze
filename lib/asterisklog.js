...
Asterisklog.prototype.print_stats=function(){
	...
	console.log("Average queue time: "+queuers.reduce((p,v)=>{return p+((v.answered-v.enqueued)/1000);},0)/queuers.length);
};
...
//	if(matches=line.match(/(SIP\/fpbx\-[a-z0-9\-]+)/i)) {
		...
		} else if (matches=line.match(/Local\/([^\@]+)\@.*connected line has changed\. Saving it until answer/)) { // app_queue.c:     -- Local/212@from-internal-a08b;1 connected line has changed. Saving it until answer for SIP/fpbx-1-f04d84a7-00000eec
