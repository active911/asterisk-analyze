...
Asterisklog.prototype.print_stats=function(){
	...
	var queuers=this.calls.filter((c)=>c.enqueued).map((c)=>{

		if(c.answered) {

			return (new Date(c.answered)-new Date(c.enqueued))/1000;	// Call was enqueued and then answered
		} else {

			if(c.end && Object.keys(c.rang).length) {

				return (new Date(c.end)-new Date(c.enqueued))/1000;		// Call was not answered, but rang at least once before it ended
			}				
		}
